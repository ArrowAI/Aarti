...
BUILD_ARGS := -trimpath -ldflags='-s -w -X "aarti.Repo=$(GITHUB_REPO)" -X "aarti.Version=$(VERSION)" -X "aarti.Commit=$(COMMIT)" -X "aarti.Date=$(COMMIT_DATE)"'
...
build-go:
	@go build $(BUILD_ARGS) -o bin/aartiserver ./cmd/aartiserver
	@go build $(BUILD_ARGS) -o bin/aarticlient ./cmd/aarticlient

install: build-ui
	@go generate ./...
	@go install $(BUILD_ARGS) ./cmd/aartiserver
	@go install $(BUILD_ARGS) ./cmd/aarticlient
...
completions:
	...
	@for shell in bash zsh fish powershell; do \
		go run ./cmd/aarticlient completion $$shell > completions/aarticlient.$$shell; \
	done
...
.PHONY: cli-docs
cli-docs:
	@rm -rf $(CLI_DOCS)/{aarticlient,aartiserver}
	@go run -tags=docs ./cmd/aarticlient docs $(CLI_DOCS)/aarticlient
	@go run -tags=docs ./cmd/aartiserver docs $(CLI_DOCS)/aartiserver
